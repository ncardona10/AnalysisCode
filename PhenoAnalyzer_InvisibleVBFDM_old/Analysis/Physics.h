...
float mjjValue(Jet *j1, Jet *j2)
{
  ...
  float mass_jj = 0.;
  if ((j1->PT > 30.0) && (j2->PT > 30.0) && (deltaR(j1,j2) > 0.3)){
    mass_jj = sqrt(2.*j1->PT*j2->PT*cosh(j1->Eta - j2->Eta));
  }
  return mass_jj;
}
                 ...
                 int entry)
{
  ...
  for (int i = 0; i < branchDict["Jet"]->GetEntries() - 1; i++)
  {
    ...
    for (int j = i + 1; j < branchDict["Jet"]->GetEntries(); j++)
    {
      ...
      if (j1->PT >= 30 && j2->PT >= 30 && abs(j1->Eta) < 5.0 && abs(j2->Eta) < 5.0 && deltaR(j1,j2) > 0.3)
      {
